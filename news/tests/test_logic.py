...
class TestCommentCreation(TestCase):
    ...
    def setUpTestData(cls) -> None:
        cls.news = News.objects.create(
            title='Зголовок новости', text='Текст новости'
        )
    ...
    def test_user_cant_use_bad_words(self):
        ...
        response = self.auth_client.post(self.url, data=bad_word_data)
...
class TestCmmentEditDelete(TestCase):
    COMMENT_TEXT = 'Текст комменатрия'
    NEW_COMMENT_TEXT = 'Обновленный комментарий'

    @classmethod
    def setUpTestData(cls) -> None:
        cls.news = News.objects.create(
            title='Тестовая новость', text='Тестовый текст'
        )
        news_id = cls.news.id
        cls.url = reverse('news:detail', args=(news_id,))
        cls.url_to_comment = cls.url + '#comments'
    ...
    def test_user_cant_delete_comment_another_user(self):
    ...
    def test_user_cant_edit_comment_another_user(self):
